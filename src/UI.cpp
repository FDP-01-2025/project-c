#include "Utilities.h"
#include "Combat.h"
#include "UI.h"
#include <iostream>
#ifdef _WIN32
#include <windows.h>
#endif

void setColor(int color)
{
#ifdef _WIN32
    HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
    SetConsoleTextAttribute(hConsole, static_cast<WORD>(color));
#else
    static const int ansiFg[16] = {
        30, 34, 32, 36, 31, 35, 33, 37, 
        90, 94, 92, 96, 91, 95, 93, 97   
    };

    if (color < 0 || color > 15) {
        std::cout << "\033[0m";
    } else {
        std::cout << "\033[" << ansiFg[color] << "m";
    }
#endif
}

void clearScreen() //Functional clear screen function for Linux and Windows
{
#ifdef _WIN32
    std::system("cls");
#else
    std::system("clear");
#endif
}

void worm(){
        cout << "                      ..   =:=.                                      " << endl;
    cout << "                  .:::::::::-+.                                      " << endl;
    cout << "                 .::::-+::::--                                       " << endl;
    cout << "                .:::::*#-:::::.                                      " << endl;
    cout << "                .::::::::::::::                                      " << endl;
    cout << "                 .:::::::::::::                                      " << endl;
    cout << "                    .:::::::::.                                      " << endl;
    cout << "                   -=::.:-+#@@*                                      " << endl;
    cout << "                   -=:-#+##*---                                      " << endl;
    cout << "                    .::---=+=-:                                      " << endl;
    cout << "                   -=:::---=#@#                                      " << endl;
    cout << "                   .=::-:@@@@#-.                                     " << endl;
    cout << "                     ..=-:---=+-.                                    " << endl;
    cout << "                    -=:::=++=--@@@-=..                               " << endl;
    cout << "                    .=-::::::=@@%:=--@@:--.::.                       " << endl;
    cout << "                       .:::-#=*-:==:#@%:=--%@+=-:.                   " << endl;
    cout << "                       ==-::.::-+-::%#--=--@@-=--@#.                 " << endl;
    cout << "                        --.:::::::.*#.-+-:=#--=:#@+:.                " << endl;
    cout << "                            :---::::::::::+:-=-.*=::.                " << endl;
    cout << "                              .    .++=...:::::-+.::.                " << endl;
    cout << "                                          .==......                  " << endl;
}

void wormDamage(){
    setColor(12); //This function changed the color to red

        cout << "                      ..   =:=.                                      " << endl;
    cout << "                  .:::::::::-+.                                      " << endl;
    cout << "                 .::::-+::::--                                       " << endl;
    cout << "                .:::::*#-:::::.                                      " << endl;
    cout << "                .::::::::::::::                                      " << endl;
    cout << "                 .:::::::::::::                                      " << endl;
    cout << "                    .:::::::::.                                      " << endl;
    cout << "                   -=::.:-+#@@*                                      " << endl;
    cout << "                   -=:-#+##*---                                      " << endl;
    cout << "                    .::---=+=-:                                      " << endl;
    cout << "                   -=:::---=#@#                                      " << endl;
    cout << "                   .=::-:@@@@#-.                                     " << endl;
    cout << "                     ..=-:---=+-.                                    " << endl;
    cout << "                    -=:::=++=--@@@-=..                               " << endl;
    cout << "                    .=-::::::=@@%:=--@@:--.::.                       " << endl;
    cout << "                       .:::-#=*-:==:#@%:=--%@+=-:.                   " << endl;
    cout << "                       ==-::.::-+-::%#--=--@@-=--@#.                 " << endl;
    cout << "                        --.:::::::.*#.-+-:=#--=:#@+:.                " << endl;
    cout << "                            :---::::::::::+:-=-.*=::.                " << endl;
    cout << "                              .    .++=...:::::-+.::.                " << endl;
    cout << "                                          .==......                  " << endl;
    setColor(7);//This function returns the text to its normal color.
}

void spider(){
    cout << "       .=@%+-..   .:@@@@@@@@+..   .:=#@@.      " << endl;
    cout << "      =@**%@@%@#*%#--%*@@@@#**@+     " << endl;
    cout << "   ..@#*#%*#@*%+::::@*@#*#%**#@.   " << endl;
    cout << "  .+%*%+.:@@%%**@-::::-@*@@@..##*@.  " << endl;
    cout << " .%#*#@###@%%%**@=::::-@**@%@%###@%*@- " << endl;
    cout << ".%#*@#**@#*#%@@%%%%@@%**%@**#@*@-" << endl;
    cout << ".@%@*@@@@#@@@@@#**%@@@@%%@#@@%*@%#." << endl;
    cout << " .%#*@++=@@%:      *#%:      -@@%=++#@**@-." << endl;
    cout << ".@*#@%**%.   .... *#%...:..   .@*#@**%+" << endl;
    cout << "%*#@%**-  .%-+@@#+%@-%.@@@+  .@**%@#**@" << endl;
    cout << "%#@@@*@#@@. =@@@@@@@#%@@@@@@%. :@%#+@*@#@@=" << endl;
    cout << " ...@*%- -@%=.@@@@@*#@@@@%..%#@. +%*#+.  " << endl;
    cout << "  .##% .@*@#%%#%*#%###@@*#..@**@:  " << endl;
    cout << "  .@*@= +#*@@#**#%##%*@@@*%=.=@**@-  " << endl;
    cout << "  .@#%#..@*@-.=@#***@#..=@*@..#@%#.  " << endl;
    cout << "    .... .@*@:  ..%@@%%@%..   :@*%.        " << endl;
    cout << "          -@*@.      ..        .@*@-         " << endl;
    cout << "          .-@*@.              .@#**@:          " << endl;
    cout << "            .@*%.             #%*.           " << endl;
    cout << "             .-@%@.            .%@*.             " << endl;

}

void spiderDamage(){
    setColor(12);
    cout << "       .=@%+-..   .:@@@@@@@@+..   .:=#@@.      " << endl;
    cout << "      =@**%@@%@#*%#--%*@@@@#**@+     " << endl;
    cout << "   ..@#*#%*#@*%+::::@*@#*#%**#@.   " << endl;
    cout << "  .+%*%+.:@@%%**@-::::-@*@@@..##*@.  " << endl;
    cout << " .%#*#@###@%%%**@=::::-@**@%@%###@%*@- " << endl;
    cout << ".%#*@#**@#*#%@@%%%%@@%**%@**#@*@-" << endl;
    cout << ".@%@*@@@@#@@@@@#**%@@@@%%@#@@%*@%#." << endl;
    cout << " .%#*@++=@@%:      *#%:      -@@%=++#@**@-." << endl;
    cout << ".@*#@%**%.   .... *#%...:..   .@*#@**%+" << endl;
    cout << "%*#@%**-  .%-+@@#+%@-%.@@@+  .@**%@#**@" << endl;
    cout << "%#@@@*@#@@. =@@@@@@@#%@@@@@@%. :@%#+@*@#@@=" << endl;
    cout << " ...@*%- -@%=.@@@@@*#@@@@%..%#@. +%*#+.  " << endl;
    cout << "  .##% .@*@#%%#%*#%###@@*#..@**@:  " << endl;
    cout << "  .@*@= +#*@@#**#%##%*@@@*%=.=@**@-  " << endl;
    cout << "  .@#%#..@*@-.=@#***@#..=@*@..#@%#.  " << endl;
    cout << "    .... .@*@:  ..%@@%%@%..   :@*%.        " << endl;
    cout << "          -@*@.      ..        .@*@-         " << endl;
    cout << "          .-@*@.              .@#**@:          " << endl;
    cout << "            .@*%.             #%*.           " << endl;
    cout << "             .-@%@.            .%@*.             " << endl;
    setColor(7);

}

void dragon(){
     cout << "       ..                                        " << endl;
    cout << "     .=.                                          " << endl;
    cout << "   .+=         .-+:.     .#.                   " << endl;
    cout << "  .#*.  .-  :##=.        :#####=.                " << endl;
    cout << "  =#- :#+.-##+.          :####+-+#*:             " << endl;
    cout << " .##=######:#######.    =.#####*:##=           " << endl;
    cout << " .########=#+#####=.     .########:##-         " << endl;
    cout << "  +########+#+######-  .    *#########=##.       " << endl;
    cout << "  ###################-    ###########*##:      " << endl;
    cout << " :####=##########%#####.:- .###############.     " << endl;
    cout << " :######*#############=  :################.    " << endl;
    cout << " .######:-######:=######+*-##################.   " << endl;
    cout << "  .############.######. +############-...-#=   " << endl;
    cout << " .-########=.  -=######=############-      #.  " << endl;
    cout << " .-#####=.    =#################-.  .+=      .:  " << endl;
    cout << "     ...     .:-#############*.       ..         " << endl;
    cout << "             -####*###########+...               " << endl;
    cout << "             -*###############*=-:.    .:.     " << endl;
    cout << "             .###*##########.#*-#=..  =#####.  " << endl;
    cout << "              .:-#==###############+. #++#*##. " << endl;
    cout << "               +:###.######*#########:  -#+ -##- " << endl;
    cout << "               -##+:.################-.  : .###: " << endl;
    cout << "                +###+#################=:-*####+  " << endl;
    cout << "             :**########-=###################.   " << endl;
    cout << "             **########-:########-  .:---:.      " << endl;
    cout << "                 .. ..    .                      " << endl;

}

void dragonDamage(){
    setColor(12);
     cout << "       ..                                        " << endl;
    cout << "     .=.                                          " << endl;
    cout << "   .+=         .-+:.     .#.                   " << endl;
    cout << "  .#*.  .-  :##=.        :#####=.                " << endl;
    cout << "  =#- :#+.-##+.          :####+-+#*:             " << endl;
    cout << " .##=######:#######.    =.#####*:##=           " << endl;
    cout << " .########=#+#####=.     .########:##-         " << endl;
    cout << "  +########+#+######-  .    *#########=##.       " << endl;
    cout << "  ###################-    ###########*##:      " << endl;
    cout << " :####=##########%#####.:- .###############.     " << endl;
    cout << " :######*#############=  :################.    " << endl;
    cout << " .######:-######:=######+*-##################.   " << endl;
    cout << "  .############.######. +############-...-#=   " << endl;
    cout << " .-########=.  -=######=############-      #.  " << endl;
    cout << " .-#####=.    =#################-.  .+=      .:  " << endl;
    cout << "     ...     .:-#############*.       ..         " << endl;
    cout << "             -####*###########+...               " << endl;
    cout << "             -*###############*=-:.    .:.     " << endl;
    cout << "             .###*##########.#*-#=..  =#####.  " << endl;
    cout << "              .:-#==###############+. #++#*##. " << endl;
    cout << "               +:###.######*#########:  -#+ -##- " << endl;
    cout << "               -##+:.################-.  : .###: " << endl;
    cout << "                +###+#################=:-*####+  " << endl;
    cout << "             :**########-=###################.   " << endl;
    cout << "             **########-:########-  .:---:.      " << endl;
    cout << "                 .. ..    .                      " << endl;
setColor(7);
}

void knight(){
    cout << ":::::::::::::-+*++++-:::::::::::----" << endl;
    cout << ":::::::::::::=*#**=::::::::-------" << endl;
    cout << ":::::::::::::-++#-:::::::::-------" << endl;
    cout << ":::::::::::::=#%%%*-::::::::::------" << endl;
    cout << ":::::::::::=*#*#*-::::::::-=+---" << endl;
    cout << "::::::::::+#++*#+::::::*##+---" << endl;
    cout << "::::::::::##%%%%%%%%#-::::#*+---" << endl;
    cout << "::::::::::#*##*#-::-##+--::-" << endl;
    cout << "::::::::::=#+++**::=#+-:::::" << endl;
    cout << ":::::::::::#*+*#+:=#**-::::::-" << endl;
    cout << ":::::::::::-#%####%%#++*-:::::---:" << endl;
    cout << "::::::::+##*#*####*-::::::::::" << endl;
    cout << ":::::::+++#%#*#*##*#-:::::::::" << endl;
    cout << ":::::::-++#+++++####*+::::::::::" << endl;
    cout << ":::::::##%*+++###%%##*#:::::" << endl;
    cout << "::::::-####%###%%####++++**=::::" << endl;
    cout << ":::::::-######+%###*#++++**-::::" << endl;
    cout << ":::::::::::=#*#%%#%#######%-::::" << endl;
    cout << "::::::::::-#######+++*##+-::::" << endl;
    cout << "::::::::::#++*###+###*-:::::" << endl;
    cout << ":::::::::::=###=---##%####-:::::::" << endl;
    cout << ":::::::::::=##+::::=##*--+-:::::::::" << endl;
    cout << ":::::::::::##*#-:::####=::::::::::::" << endl;
    cout << ":::::::::::#*+::::+##=::::::::::::" << endl;
    cout << ".::::::::::#+=::::-#+=::::::::::::" << endl;
    cout << ":.:.::::::-##=::::-#*+-:::::::::::" << endl;
    cout << "...:::.:=*#+-:::::+*####=:::::::::" << endl;

}

void knightDamage(){
    setColor(12);
    cout << ":::::::::::::-+*++++-:::::::::::----" << endl;
    cout << ":::::::::::::=*#**=::::::::-------" << endl;
    cout << ":::::::::::::-++#-:::::::::-------" << endl;
    cout << ":::::::::::::=#%%%*-::::::::::------" << endl;
    cout << ":::::::::::=*#*#*-::::::::-=+---" << endl;
    cout << "::::::::::+#++*#+::::::*##+---" << endl;
    cout << "::::::::::##%%%%%%%%#-::::#*+---" << endl;
    cout << "::::::::::#*##*#-::-##+--::-" << endl;
    cout << "::::::::::=#+++**::=#+-:::::" << endl;
    cout << ":::::::::::#*+*#+:=#**-::::::-" << endl;
    cout << ":::::::::::-#%####%%#++*-:::::---:" << endl;
    cout << "::::::::+##*#*####*-::::::::::" << endl;
    cout << ":::::::+++#%#*#*##*#-:::::::::" << endl;
    cout << ":::::::-++#+++++####*+::::::::::" << endl;
    cout << ":::::::##%*+++###%%##*#:::::" << endl;
    cout << "::::::-####%###%%####++++**=::::" << endl;
    cout << ":::::::-######+%###*#++++**-::::" << endl;
    cout << ":::::::::::=#*#%%#%#######%-::::" << endl;
    cout << "::::::::::-#######+++*##+-::::" << endl;
    cout << "::::::::::#++*###+###*-:::::" << endl;
    cout << ":::::::::::=###=---##%####-:::::::" << endl;
    cout << ":::::::::::=##+::::=##*--+-:::::::::" << endl;
    cout << ":::::::::::##*#-:::####=::::::::::::" << endl;
    cout << ":::::::::::#*+::::+##=::::::::::::" << endl;
    cout << ".::::::::::#+=::::-#+=::::::::::::" << endl;
    cout << ":.:.::::::-##=::::-#*+-:::::::::::" << endl;
    cout << "...:::.:=*#+-:::::+*####=:::::::::" << endl;
setColor(7);
}

void slime(){

    cout << "                     ----------------                   " << endl;
    cout << "                  -----=++++++++---=---                 " << endl;
    cout << "                ----=++++=------   -++---               " << endl;
    cout << "               ---=+++=-------------=+++--              " << endl;
    cout << "              ---++++=----------------+++--             " << endl;
    cout << "             ---+@%#+-----------------#%@+--            " << endl;
    cout << "            ---++@@*=------------------@@+---           " << endl;
    cout << "           .--++++++=--------------------++--           " << endl;
    cout << "           ---++++++=--------------------+++--          " << endl;
    cout << "          :--=+++++++=------------------++++--          " << endl;
    cout << "          ---++++++--------------------+++++---         " << endl;
    cout << "         ---+++++++=-----------------=+++++++---        " << endl;
    cout << "        :---+++=--+++==++++===--==+++++++--==----       " << endl;
    cout << "       ----+++++==+++++++++++++++++++++++--=+-----      " << endl;
    cout << "     ----+++++++=====----------------=====++++++----    " << endl;
    cout << "    ---=++++=-------------------------------=+++++---   " << endl;
    cout << "    :-----=++==----------------------------=+++=----    " << endl;
    cout << "       ----------------===============------------      " << endl;
    cout << "               :-------------------------.              " << endl;

}

void slimeDamage(){
    setColor(12);
    cout << "                     ----------------                   " << endl;
    cout << "                  -----=++++++++---=---                 " << endl;
    cout << "                ----=++++=------   -++---               " << endl;
    cout << "               ---=+++=-------------=+++--              " << endl;
    cout << "              ---++++=----------------+++--             " << endl;
    cout << "             ---+@%#+-----------------#%@+--            " << endl;
    cout << "            ---++@@*=------------------@@+---           " << endl;
    cout << "           .--++++++=--------------------++--           " << endl;
    cout << "           ---++++++=--------------------+++--          " << endl;
    cout << "          :--=+++++++=------------------++++--          " << endl;
    cout << "          ---++++++--------------------+++++---         " << endl;
    cout << "         ---+++++++=-----------------=+++++++---        " << endl;
    cout << "        :---+++=--+++==++++===--==+++++++--==----       " << endl;
    cout << "       ----+++++==+++++++++++++++++++++++--=+-----      " << endl;
    cout << "     ----+++++++=====----------------=====++++++----    " << endl;
    cout << "    ---=++++=-------------------------------=+++++---   " << endl;
    cout << "    :-----=++==----------------------------=+++=----    " << endl;
    cout << "       ----------------===============------------      " << endl;
    cout << "               :-------------------------.              " << endl;
setColor(7);
}

void orc(){
        cout << "          #=+.                           " << endl;
    cout << "         ##+++++.                          " << endl;
    cout << "        ###*#-                        " << endl;
    cout << "        =####%#%*.         =#             " << endl;
    cout << "        =###%###.      -####               " << endl;
    cout << "        -#%###   =. :=---=                 " << endl;
    cout << "        -+=-+   .+#+=@==%+-+=+             " << endl;
    cout << "       -+-===#+-=+#+-%--*-==-+----=        " << endl;
    cout << "       =-=+*+=+#=%-+#%-+#+=-------=:     " << endl;
    cout << "     =##+-===*+=#++#*#+-=--=--==-=--:  " << endl;
    cout << "   -=#%##%@@@+++++##====+=----*---=+---  " << endl;
    cout << "  -+=@%%@@@@@++---==*=--------=--*=-=+ " << endl;
    cout << "  ===+#@@@%@#*+================#+++++==+ " << endl;
    cout << " ==+==%# **+====++========+  *+++=== " << endl;
    cout << "=+++###+       ####+#++*#+    #@#####* " << endl;
    cout << "=#+.          *#####++*++##   @@@###@%-" << endl;
    cout << "               %%%###+##%%%##  ##*#@%#." << endl;
    cout << "              ####%%%%%%########*##**  " << endl;
    cout << "             ####@#%*@#*#-######*  " << endl;
    cout << "             ##@##@+++%@++++ ..:.    " << endl;
    cout << "             ###==+=@+===#*====%         " << endl;
    cout << "             *===+= *= =====--=.         " << endl;
    cout << "               .---+   *=-+--:           " << endl;
    cout << "                +====-    +---=            " << endl;
    cout << "               ++===*.  ======           " << endl;
    cout << "           -+++++++#. .**+======+:       " << endl;
    cout << "          -*####-             =%###+        " << endl;
}

void orcDamage(){
    setColor(12);
        cout << "          #=+.                           " << endl;
    cout << "         ##+++++.                          " << endl;
    cout << "        ###*#-                        " << endl;
    cout << "        =####%#%*.         =#             " << endl;
    cout << "        =###%###.      -####               " << endl;
    cout << "        -#%###   =. :=---=                 " << endl;
    cout << "        -+=-+   .+#+=@==%+-+=+             " << endl;
    cout << "       -+-===#+-=+#+-%--*-==-+----=        " << endl;
    cout << "       =-=+*+=+#=%-+#%-+#+=-------=:     " << endl;
    cout << "     =##+-===*+=#++#*#+-=--=--==-=--:  " << endl;
    cout << "   -=#%##%@@@+++++##====+=----*---=+---  " << endl;
    cout << "  -+=@%%@@@@@++---==*=--------=--*=-=+ " << endl;
    cout << "  ===+#@@@%@#*+================#+++++==+ " << endl;
    cout << " ==+==%# **+====++========+  *+++=== " << endl;
    cout << "=+++###+       ####+#++*#+    #@#####* " << endl;
    cout << "=#+.          *#####++*++##   @@@###@%-" << endl;
    cout << "               %%%###+##%%%##  ##*#@%#." << endl;
    cout << "              ####%%%%%%########*##**  " << endl;
    cout << "             ####@#%*@#*#-######*  " << endl;
    cout << "             ##@##@+++%@++++ ..:.    " << endl;
    cout << "             ###==+=@+===#*====%         " << endl;
    cout << "             *===+= *= =====--=.         " << endl;
    cout << "               .---+   *=-+--:           " << endl;
    cout << "                +====-    +---=            " << endl;
    cout << "               ++===*.  ======           " << endl;
    cout << "           -+++++++#. .**+======+:       " << endl;
    cout << "          -*####-             =%###+        " << endl;
    setColor(7);
}

void snake(){
       cout << "                                            =++%   " << endl;
    cout << "                                        ..  %=-=@  " << endl;
    cout << "                          :@. #######%##%@@--=--=- " << endl;
    cout << "                         ##== .#########:::....  * " << endl;
    cout << "        =#@=#####+      #####  -#####:-###++:# " << endl;
    cout << "     -#####*.-####+.%+  -######=    ..  -########+ " << endl;
    cout << "    #######: +## :###*+#######:     +#########.  " << endl;
    cout << "  #######-:: =#*  .############*  =##########.     " << endl;
    cout << "  ######-::. +##=.########=####= =#*#########.   " << endl;
    cout << " .#####*::##= ....=########## +#+  *#############  " << endl;
    cout << "  ####- .###: ..:+###########-    -=: .+######*% " << endl;
    cout << "  @++-     #.  =@    :=. :##: :#*       .=+- ..@:" << endl;
    cout << "  %-. -### @.    =@    .==:... :#   .+**+:..:. *+" << endl;
    cout << "   ######- .@      .##############* ##############*" << endl;
    cout << "   .##:     -@      -##########..=##=-----==-+##:" << endl;
    cout << "     %.      ::.@       -#######..::::::=###*- .@" << endl;
    cout << "    .    :++###         *####. =##############+  " << endl;
    cout << "    +-    =######            #@-*#############=    " << endl;
    cout << "     .#.    ######@              .=########*-      " << endl;
    cout << "     -#. @. *###-@                                 " << endl;
    cout << "       -@%.  .-+-:@                                " << endl;
    cout << "           @+ ...=@                                " << endl;
    cout << "             -@%%*                                 " << endl;
}

void snakeDamage(){
    setColor(12);
       cout << "                                            =++%   " << endl;
    cout << "                                        ..  %=-=@  " << endl;
    cout << "                          :@. #######%##%@@--=--=- " << endl;
    cout << "                         ##== .#########:::....  * " << endl;
    cout << "        =#@=#####+      #####  -#####:-###++:# " << endl;
    cout << "     -#####*.-####+.%+  -######=    ..  -########+ " << endl;
    cout << "    #######: +## :###*+#######:     +#########.  " << endl;
    cout << "  #######-:: =#*  .############*  =##########.     " << endl;
    cout << "  ######-::. +##=.########=####= =#*#########.   " << endl;
    cout << " .#####*::##= ....=########## +#+  *#############  " << endl;
    cout << "  ####- .###: ..:+###########-    -=: .+######*% " << endl;
    cout << "  @++-     #.  =@    :=. :##: :#*       .=+- ..@:" << endl;
    cout << "  %-. -### @.    =@    .==:... :#   .+**+:..:. *+" << endl;
    cout << "   ######- .@      .##############* ##############*" << endl;
    cout << "   .##:     -@      -##########..=##=-----==-+##:" << endl;
    cout << "     %.      ::.@       -#######..::::::=###*- .@" << endl;
    cout << "    .    :++###         *####. =##############+  " << endl;
    cout << "    +-    =######            #@-*#############=    " << endl;
    cout << "     .#.    ######@              .=########*-      " << endl;
    cout << "     -#. @. *###-@                                 " << endl;
    cout << "       -@%.  .-+-:@                                " << endl;
    cout << "           @+ ...=@                                " << endl;
    cout << "             -@%%*                                 " << endl;
    setColor(7);
}

void mouse(){
    cout << "      :%@+==+#@:        :@#==*@+      " << endl;
    cout << "    .@+==+#%%=#      *%=+#@%+=@.    " << endl;
    cout << "    #==@+====@=@:.---=@#====#%=#    " << endl;
    cout << "    @=+@======@%=====+@#=====@=%    " << endl;
    cout << "    +*=@+============+=+=%====#%%-    " << endl;
    cout << "     =%+#%+==+======#@@+=%#@%%      " << endl;
    cout << "       :%%%@@@============+*##+.   " << endl;
    cout << "        .....==+#%=====@***@=%:. " << endl;
    cout << "            ..=#==##=====#@@@@@%%+   " << endl;
    cout << ":.:+@*      .     .%@%*++#%%@=:=.#:  " << endl;
    cout << "    .@:          #+====+@:.=#*##-    " << endl;
    cout << "     =@         %+======@.           " << endl;
    cout << "     +%       :@+==*=##@*@-        " << endl;
    cout << "     @.      ##==+++@#%%=#%@=        " << endl;
    cout << "   .@:      @+=#%#===*@===%@+        " << endl;
    cout << "   @.      +=====+@======@+@       " << endl;
    cout << "  @=       #========@+=====@+%-      " << endl;
    cout << " .@        +*=======@+====+%=@.      " << endl;
    cout << "  @=        +#=====%#===+@*+@:       " << endl;
    cout << "   #@+.  .@%:%==@@@@@@+==#@%@*     " << endl;
    cout << "      .::.    -@@#+==+%*@@@%%@@%.    " << endl;
}

void mouseDamage(){
    setColor(12);
    cout << "      :%@+==+#@:        :@#==*@+      " << endl;
    cout << "    .@+==+#%%=#      *%=+#@%+=@.    " << endl;
    cout << "    #==@+====@=@:.---=@#====#%=#    " << endl;
    cout << "    @=+@======@%=====+@#=====@=%    " << endl;
    cout << "    +*=@+============+=+=%====#%%-    " << endl;
    cout << "     =%+#%+==+======#@@+=%#@%%      " << endl;
    cout << "       :%%%@@@============+*##+.   " << endl;
    cout << "        .....==+#%=====@***@=%:. " << endl;
    cout << "            ..=#==##=====#@@@@@%%+   " << endl;
    cout << ":.:+@*      .     .%@%*++#%%@=:=.#:  " << endl;
    cout << "    .@:          #+====+@:.=#*##-    " << endl;
    cout << "     =@         %+======@.           " << endl;
    cout << "     +%       :@+==*=##@*@-        " << endl;
    cout << "     @.      ##==+++@#%%=#%@=        " << endl;
    cout << "   .@:      @+=#%#===*@===%@+        " << endl;
    cout << "   @.      +=====+@======@+@       " << endl;
    cout << "  @=       #========@+=====@+%-      " << endl;
    cout << " .@        +*=======@+====+%=@.      " << endl;
    cout << "  @=        +#=====%#===+@*+@:       " << endl;
    cout << "   #@+.  .@%:%==@@@@@@+==#@%@*     " << endl;
    cout << "      .::.    -@@#+==+%*@@@%%@@%.    " << endl;
    setColor(7);
}

void principalMenu(){
    bool menu = true;
    int opcion;


    do {

    clearScreen();

    cout<<""<<endl;
    cout<<""<<endl;
    cout << "██████╗░░█████╗░███╗░░██╗██████╗░░█████╗░███╗░░░███╗  ██████╗░██╗░░░██╗███╗░░██╗░██████╗░███████╗░█████╗░███╗░░██╗" << endl;
    cout << "██╔══██╗██╔══██╗████╗░██║██╔══██╗██╔══██╗████╗░████║  ██╔══██╗██║░░░██║████╗░██║██╔════╝░██╔════╝██╔══██╗████╗░██║" << endl;
    cout << "██████╔╝███████║██╔██╗██║██║░░██║██║░░██║██╔████╔██║  ██║░░██║██║░░░██║██╔██╗██║██║░░██╗░█████╗░░██║░░██║██╔██╗██║" << endl;
    cout << "██╔══██╗██╔══██║██║╚████║██║░░██║██║░░██║██║╚██╔╝██║  ██║░░██║██║░░░██║██║╚████║██║░░╚██╗██╔══╝░░██║░░██║██║╚████║" << endl;
    cout << "██║░░██║██║░░██║██║░╚███║██████╔╝╚█████╔╝██║░╚═╝░██║  ██████╔╝╚██████╔╝██║░╚███║╚██████╔╝███████╗╚█████╔╝██║░╚███║" << endl;
    cout << "╚═╝░░╚═╝╚═╝░░╚═╝╚═╝░░╚══╝╚═════╝░░╚════╝░╚═╝░░░░░╚═╝  ╚═════╝░░╚═════╝░╚═╝░░╚══╝░╚═════╝░╚══════╝░╚════╝░╚═╝░░╚══╝" << endl;
     cout<<""<<endl;
    cout<<""<<endl;
    cout << "                                                ▄█   ▀  █▀█ █░░ ▄▀█ █▄█" << endl;
    cout << "                                                ░█   ▄  █▀▀ █▄▄ █▀█ ░█░" << endl;
    cout<<""<<endl;                                                  
    cout<<"                                     ▀█  ▄  █ █▄░█ █▀▀ █ █▄░█ █ ▀█▀ █▀▀   █▀▄▀█ █▀█ █▀▄ █▀▀"<<endl;
    cout<<"                                     █▄  ▄  █ █░▀█ █▀░ █ █░▀█ █ ░█░ ██▄   █░▀░█ █▄█ █▄▀ ██▄"<<endl;
    cout<<""<<endl;
    cout << "                                                 ▀█   ▀  █▀▀ ▀▄▀ █ ▀█▀" << endl;
    cout << "                                                 ▄█   ▄  ██▄ █░█ █ ░█░ " << endl;
    cin>>opcion;
    cinControl();

    switch(opcion){

    case 1:
            game();
    break;
    

    case 3:
        cout << R"(

        █▀▀ ▄▀█ █▀▄▀█ █▀▀   █▀█ █░█ █▀▀ █▀█
        █▄█ █▀█ █░▀░█ ██▄   █▄█ ▀▄▀ ██▄ █▀▄

)";


        exit(0);
        menu = false;
        break;

        case 2:
            loadingScreen();
            infiniteMode();
            break;


    default:
        cout<<"Isn't a valid option"<<endl;
    break;
        }
    }while(menu);
}

void showEnemy(int playerLevel){
    switch (playerLevel) {
    case 1: worm(); break;
    case 2: spider(); break;
    case 3: dragon(); break;
    case 4: slime(); break;
    case 5: snake(); break;
    case 6: knight(); break;
    case 7: orc(); break;
    case 8: snake(); break;
    case 9: mouse(); break;
    case 10:orc(); break;
}

}

void showEnemyDamage(int playerLevel){
    switch (playerLevel) {
    case 1: wormDamage(); break;
    case 2: spiderDamage(); break;
    case 3: dragonDamage(); break;
    case 4: slimeDamage(); break;
    case 5: snakeDamage(); break;
    case 6: knightDamage(); break;
    case 7: orcDamage(); break;
    case 8: snakeDamage(); break;
    case 9: mouseDamage(); break;
    case 10:orcDamage(); break;
 
}

}

void animationDamage(void (*normal)(int), void (*damage)(int), int nivel) {
    //Takes function parameters
   clearScreen();
    damage(nivel);
    timer(1);
    clearScreen();
    normal(nivel);
}

void firstGame(int option, Player& player){

    switch (option){
    case 1:
     clearScreen();
    showEnemy(player.lvl);
    cout<< R"(
    █▀ █▀▀ █░░ █▀▀ █▀▀ ▀█▀   ▄▀█   █▄░█ █░█ █▀▄▀█ █▄▄ █▀▀ █▀█ ▄
    ▄█ ██▄ █▄▄ ██▄ █▄▄ ░█░   █▀█   █░▀█ █▄█ █░▀░█ █▄█ ██▄ █▀▄ ▄
    )";
    break;

    case 2:
    cout<< R"(
    ▄█     ▀█     ▀█
░    █     █▄     ▄█
    )";
    break;

    case 3:
        cout<< R"(
    ▄█     ▀█     ▀█     █░█     █▀
░    █     █▄     ▄█     ▀▀█     ▄█
    )";
    break;
    
    case 4 :
    clearScreen();  
    showEnemy(player.lvl);
    setColor(12);
 cout << R"(
    █ █▄░█ █░█ ▄▀█ █░░ █ █▀▄  █▄░█ █░█ █▀▄▀█ █▄▄ █▀▀ █▀█
    █ █░▀█ ▀▄▀ █▀█ █▄▄ █ █▄▀  █░▀█ █▄█ █░▀░█ █▄█ ██▄ █▀▄
)"; 
setColor(7);
    break;
    
    case 5:
   clearScreen();  
    showEnemy(player.lvl);
    setColor(12);
       cout << R"(
    █▀▀ █▄░█ █▀▀ █▀▄▀█ █▄█   ▄▀█ ▀█▀ ▀█▀ ▄▀█ █▀▀ █▄▀ █▀   █▄█ █▀█ █░█
    ██▄ █░▀█ ██▄ █░▀░█ ░█░   █▀█ ░█░ ░█░ █▀█ █▄▄ █░█ ▄█   ░█░ █▄█ █▄█
)"; 
setColor(7);
timer(1);
    break;

    case 6:
    clearScreen(); 
    showEnemy(player.lvl);
    animationDamage(showEnemy, showEnemyDamage, player.lvl);
         cout << R"(
    █▄█ █▀█ █░█   ▄▀█ ▀█▀ ▀█▀ ▄▀█ █▀▀ █▄▀   █▀▀ █▄░█ █▀▀ █▀▄▀█ █▄█
    ░█░ █▄█ █▄█   █▀█ ░█░ ░█░ █▀█ █▄▄ █░█   ██▄ █░▀█ ██▄ █░▀░█ ░█░
)";
timer(1);
    break;
    }
 
}

void secondGame(int option, Player& player){
   
    
    switch(option){
        case 1 :
        clearScreen();
    showEnemy(player.lvl);
        cout << R"(
    █▀▄▀█ █▀▀ █▀▄▀█ █▀█ █▀█ █ ▀█ █▀▀   ▀█▀ █░█ █▀▀   █▄░█ █░█ █▀▄▀█ █▄▄ █▀▀ █▀█ █▀ ▀
    █░▀░█ ██▄ █░▀░█ █▄█ █▀▄ █ █▄ ██▄   ░█░ █▀█ ██▄   █░▀█ █▄█ █░▀░█ █▄█ ██▄ █▀▄ ▄█ ▄
)";
        break;
     
        case 2:
        cout << R"(
    █▄░█ █░█ █▀▄▀█ █▄▄ █▀▀ █▀█ █▀   █▀▄ █ █▀ ▄▀█ █▀█ █▀█ █▀▀ ▄▀█ █▀█   █ █▄░█ ▀
    █░▀█ █▄█ █░▀░█ █▄█ ██▄ █▀▄ ▄█   █▄▀ █ ▄█ █▀█ █▀▀ █▀▀ ██▄ █▀█ █▀▄   █ █░▀█ ▄
)";
        break;

        case 3:
        clearScreen();
    showEnemy(player.lvl);
        cout << R"(
    █░█░█ █▀█ █ ▀█▀ █▀▀   ▀█▀ █░█ █▀▀   █▄░█ █░█ █▀▄▀█ █▄▄ █▀▀ █▀█ █▀ ▀
    ▀▄▀▄▀ █▀▄ █ ░█░ ██▄   ░█░ █▀█ ██▄   █░▀█ █▄█ █░▀░█ █▄█ ██▄ █▀▄ ▄█ ▄
)";

        break;

        case 4:
        animationDamage(showEnemy, showEnemyDamage, player.lvl);
        cout << R"(
    █▀▀ █▀█ █▀█ █▀█ █▀▀ █▀▀ ▀█▀ █
    █▄▄ █▄█ █▀▄ █▀▄ ██▄ █▄▄ ░█░ ▄
)";
timer(1);
        break;

        case 5:
        clearScreen();
        showEnemy(player.lvl);
        setColor(12);
        cout << R"(
    █░█░█ █▀█ █▀█ █▄░█ █▀▀ █
    ▀▄▀▄▀ █▀▄ █▄█ █░▀█ █▄█ ▄
)";
setColor(7);
timer(1);
        break;

        case 6:
    clearScreen();
    showEnemy(player.lvl);
        setColor(12);
        cout << R"(
    █▀▀ █▄░█ █▀▀ █▀▄▀█ █▄█   █ █▀   █▀▀ █▀█ █▄░█ █▄░█ ▄▀█   █▄▀ █ █░░ █░░   █▄█ █▀█ █░█
    ██▄ █░▀█ ██▄ █░▀░█ ░█░   █ ▄█   █▄█ █▄█ █░▀█ █░▀█ █▀█   █░█ █ █▄▄ █▄▄   ░█░ █▄█ █▄█
)";
setColor(7);
        break;

        case 7:
        animationDamage(showEnemy, showEnemyDamage, player.lvl);
          cout << R"(
    █▄█ █▀█ █░█ ▀ █░█ █▀▀   █▀▄ █▀▀ █▀▀ █▀▀ ▄▀█ ▀█▀ █▀▀ █▀▄   ▀█▀ █░█ █▀▀   █▀▀ █▄░█ █▀▀ █▀▄▀█ █▄█ █
    ░█░ █▄█ █▄█ ░ ▀▄▀ ██▄   █▄▀ ██▄ █▀░ ██▄ █▀█ ░█░ ██▄ █▄▀   ░█░ █▀█ ██▄   ██▄ █░▀█ ██▄ █░▀░█ ░█░ ▄
)";
timer(1);
        break;

    }
}

void thirdGame(int option, Player& player){
    
    switch(option){
        case 1 :
        cout << R"(
    █ ▀█▀   █░░ █▀█ █▀█ █▄▀ █▀   █░░ █ █▄▀ █▀▀   █▀▀ █▄░█ █▀▀ █▀▄▀█ █▄█   █ ▀█▀ ▀ █▀   █▀▀ █▀█ █▄░█ ▄▀█ █▀▄▀█ ▄▀█ █▄▀ █▀▀   ▄▀█ 
    █ ░█░   █▄▄ █▄█ █▄█ █░█ ▄█   █▄▄ █ █░█ ██▄   ██▄ █░▀█ ██▄ █░▀░█ ░█░   █ ░█░ ░ ▄█   █▄█ █▄█ █░▀█ █▀█ █░▀░█ █▀█ █░█ ██▄   █▀█ 

    █░█░█ █▀▀ ▄▀█ █▄▀   ▄▀█ ▀█▀ ▀█▀ ▄▀█ █▀▀ █▄▀
    ▀▄▀▄▀ ██▄ █▀█ █░█   █▀█ ░█░ ░█░ █▀█ █▄▄ █░█
)";
        break;

        case 2:
        cout << R"(
    █ ▀█▀   █░░ █▀█ █▀█ █▄▀ █▀   █░░ █ █▄▀ █▀▀   █▀▀ █▄░█ █▀▀ █▀▄▀█ █▄█   █ ▀█▀ ▀ █▀   █▀▀ █▀█ █▄░█ ▄▀█ █▀▄▀█ ▄▀█ █▄▀ █▀▀   ▄▀█
    █ ░█░   █▄▄ █▄█ █▄█ █░█ ▄█   █▄▄ █ █░█ ██▄   ██▄ █░▀█ ██▄ █░▀░█ ░█░   █ ░█░ ░ ▄█   █▄█ █▄█ █░▀█ █▀█ █░▀░█ █▀█ █░█ ██▄   █▀█

    █▀ ▀█▀ █▀█ █▀█ █▄░█ █▀▀   ▄▀█ ▀█▀ ▀█▀ ▄▀█ █▀▀ █▄▀
    ▄█ ░█░ █▀▄ █▄█ █░▀█ █▄█   █▀█ ░█░ ░█░ █▀█ █▄▄ █░█
)";     
        break;

        case 3:
        cout << R"(
    █ ▀█▀   █░░ █▀█ █▀█ █▄▀ █▀   █░░ █ █▄▀ █▀▀   █▀▀ █▄░█ █▀▀ █▀▄▀█ █▄█   █ ▀█▀ ▀ █▀   █▀▀ █▀█ █▄░█ ▄▀█   █░█ █▀ █▀▀
    █ ░█░   █▄▄ █▄█ █▄█ █░█ ▄█   █▄▄ █ █░█ ██▄   ██▄ █░▀█ ██▄ █░▀░█ ░█░   █ ░█░ ░ ▄█   █▄█ █▄█ █░▀█ █▀█   █▄█ ▄█ ██▄

    █▀█ █▀█ █▀█ ▀█▀ █▀▀ █▀▀ ▀█▀ █ █▀█ █▄░█
    █▀▀ █▀▄ █▄█ ░█░ ██▄ █▄▄ ░█░ █ █▄█ █░▀█
)";
        break;

        case 4:
        clearScreen();
    showEnemy(player.lvl);
         cout << R"(
    █▀▀ █░█ █▀█ █▀█ █▀ █▀▀   ▄▀█ █▄░█   ▄▀█ █▀▀ ▀█▀ █ █▀█ █▄░█ ▀
    █▄▄ █▀█ █▄█ █▄█ ▄█ ██▄   █▀█ █░▀█   █▀█ █▄▄ ░█░ █ █▄█ █░▀█ ▄

    ▄█ ░   █░█░█ █▀▀ ▄▀█ █▄▀   ▄▀█ ▀█▀ ▀█▀ ▄▀█ █▄▀             ▀█ ░   █▀▄ █▀▀ █▀▀ █▀▀ █▄░█ █▀ █▀▀
    ░█ ▄   ▀▄▀▄▀ ██▄ █▀█ █░█   █▀█ ░█░ ░█░ █▀█ █░█             ▄█ ▄   █▄▀ ██▄ █▀░ ██▄ █░▀█ ▄█ ██▄

    ▀█ ░   █▀ ▀█▀ █▀█ █▀█ █▄░█ █▀▀   ▄▀█ ▀█▀ ▀█▀ ▄▀█ █▄▀  
    █▄ ▄   ▄█ ░█░ █▀▄ █▄█ █░▀█ █▄█   █▀█ ░█░ ░█░ █▀█ █░█  
)";
        break;

        case 5:
        clearScreen();
    showEnemy(player.lvl);
        setColor(12);
        cout << R"(
    █ █▄░█ █░█ ▄▀█ █░░ █ █▀▄   █▄░█ █░█ █▀▄▀█ █▄▄ █▀▀ █▀█
    █ █░▀█ ▀▄▀ █▀█ █▄▄ █ █▄▀   █░▀█ █▄█ █░▀░█ █▄█ ██▄ █▀▄
)";
setColor(7);
        break;

        case 6:
        clearScreen();
    showEnemy(player.lvl);
     cout << R"(
    █▀ ▄▀█ █▀▄▀█ █▀▀   ▄▀█ █▀▀ ▀█▀ █ █▀█ █▄░█ ░   █▄░█ █▀█   █▀█ █▄░█ █▀▀   █░░ █▀█ █▀ █▀▀ █▀   █░░ █ █▀▀ █▀▀
    ▄█ █▀█ █░▀░█ ██▄   █▀█ █▄▄ ░█░ █ █▄█ █░▀█ █   █░▀█ █▄█   █▄█ █░▀█ ██▄   █▄▄ █▄█ ▄█ ██▄ ▄█   █▄▄ █ █▀░ ██▄
)";

        break;

        case 7:
        animationDamage(showEnemy, showEnemyDamage, player.lvl);
    cout << R"(
    █▄█ █▀█ █░█   ▄▀█ ▀█▀ ▀█▀ ▄▀█ █▀▀ █▄▀   █▀▀ █▄░█ █▀▀ █▀▄▀█ █▄█
    ░█░ █▄█ █▄█   █▀█ ░█░ ░█░ █▀█ █▄▄ █░█   ██▄ █░▀█ ██▄ █░▀░█ ░█░
)";
        break;

        case 8:
        clearScreen(); 
    showEnemy(player.lvl);
        setColor(12);
 cout << R"(
    █▀▀ █▄░█ █▀▀ █▀▄▀█ █▄█   ▄▀█ ▀█▀ ▀█▀ ▄▀█ █▀▀ █▄▀ █▀   █▄█ █▀█ █░█
    ██▄ █░▀█ ██▄ █░▀░█ ░█░   █▀█ ░█░ ░█░ █▀█ █▄▄ █░█ ▄█   ░█░ █▄█ █▄█
)"; 
setColor(7);
        break;
    }
}

void UTF8Config() {
#ifdef _WIN32
    SetConsoleOutputCP(CP_UTF8); //For Windows
#else
#endif
}

void loadingScreen(){

    clearScreen();
     cout << R"(

        █░░ █▀█ ▄▀█ █▀▄ █ █▄░█ █▀▀  
        █▄▄ █▄█ █▀█ █▄▀ █ █░▀█ █▄█   ▄ ▄ ▄

        ████▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒
        ████▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒
        ████▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒
)";

    timer(1);


    clearScreen();
 cout << R"(
     
        █░░ █▀█ ▄▀█ █▀▄ █ █▄░█ █▀▀   ▄ ▄ ▄
        █▄▄ █▄█ █▀█ █▄▀ █ █░▀█ █▄█  

        █████████████████████▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒
        █████████████████████▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒
        █████████████████████▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒
)";
    timer(1);

        clearScreen();
 cout << R"(
     
        █░░ █▀█ ▄▀█ █▀▄ █ █▄░█ █▀▀   
        █▄▄ █▄█ █▀█ █▄▀ █ █░▀█ █▄█   ▄ ▄ ▄

        ██████████████████████████████████████████
        ██████████████████████████████████████████
        ██████████████████████████████████████████
)";
    timer(1);
    clearScreen();


}

void secondMenu(int option, Player& player, Enemy& enemy)
{

    switch (option)
    {
    case 1:
        cout<<""<<endl;
        cout<<""<<endl;
        cout<<" ＳＥＬＥＣＴ ＡＮ ＯＰＴＩＯＮ："<<endl;
        cout<<""<<endl;
        cout<<"１．ＦＩＧＨＴ     ２．ＵＳＥ ＰＯＴＩＯＮ ："
        << player.items << "     ３．ＥＸＩＴ" <<endl;
        break;

    case 2:
        cout<<"ＳＴＡＴＩＳＴＩＣＳ ："<<endl;
        cout<<""<<endl;
        cout << "ＬＩＦＥ ：" << player.life << '/' << player.lifeMax 
        <<"     ＥＮＥＭＹ ＬＩＦＥ ：" << enemy.life << endl;
        break;

    case 3:
        cout<<""<<endl;
        cout << "ＤＡＭＡＧＥ ： "<< player.damage
        << "     ＥＮＥＭＹ ＤＡＭＡＧＥ ："<< enemy.damage << endl;
        break;
    }
}

void invalid(){
     cout<<R"(
     
█ ▀█▀   █ █▀ █▄░█ ▀ ▀█▀   ▄▀█   █░█ ▄▀█ █░░ █ █▀▄   █▀█ █▀█ ▀█▀ █ █▀█ █▄░█
█ ░█░   █ ▄█ █░▀█ ░ ░█░   █▀█   ▀▄▀ █▀█ █▄▄ █ █▄▀   █▄█ █▀▀ ░█░ █ █▄█ █░▀█
     
     )";
}

void itemsText(int option){

    switch(option){

        case 1:
        cout<<R"(
     
        
█▄█ █▀█ █░█   █░█░█ █ █▄░█   ▄▀█   █░░ █ █▀▀ █▀▀   █▀▄▀█ ▄▀█ ▀▄▀   █ █▄░█ █▀▀ █▀█ █▀▀ ▄▀█ █▀ █▀▀   █
░█░ █▄█ █▄█   ▀▄▀▄▀ █ █░▀█   █▀█   █▄▄ █ █▀░ ██▄   █░▀░█ █▀█ █░█   █ █░▀█ █▄▄ █▀▄ ██▄ █▀█ ▄█ ██▄   ▄
     )";
break;
     case 2:
        cout<<R"(
     
        
█▄█ █▀█ █░█   █░█░█ █ █▄░█   ▄▀█   █▀▄ ▄▀█ █▀▄▀█ ▄▀█ █▀▀ █▀▀   █ █▄░█ █▀▀ █▀█ █▀▀ ▄▀█ █▀ █▀▀   █
░█░ █▄█ █▄█   ▀▄▀▄▀ █ █░▀█   █▀█   █▄▀ █▀█ █░▀░█ █▀█ █▄█ ██▄   █ █░▀█ █▄▄ █▀▄ ██▄ █▀█ ▄█ ██▄   ▄
     )";
break;
     case 3:
        cout<<R"(
     
        
█▄█ █▀█ █░█   █░█░█ █ █▄░█   ▄▀█   █░░ █ █▀▀ █▀▀   █▀█ █▀█ ▀█▀ █ █▀█ █▄░█   █
░█░ █▄█ █▄█   ▀▄▀▄▀ █ █░▀█   █▀█   █▄▄ █ █▀░ ██▄   █▀▀ █▄█ ░█░ █ █▄█ █░▀█   ▄
     )";
break;
     case 4:
        cout<<R"(
     
        
█▄█ █▀█ █░█ ▀ █░█ █▀▀   █░█░█ █ █▄░█   ▀█▀ █░█ █▀▀   █▀▀ █▀█ █▀▀ ▄▀█ ▀█▀   ░░█ ▄▀█ █▀▀ █▄▀ █▀█ █▀█ ▀█▀   █ █
░█░ █▄█ █▄█ ░ ▀▄▀ ██▄   ▀▄▀▄▀ █ █░▀█   ░█░ █▀█ ██▄   █▄█ █▀▄ ██▄ █▀█ ░█░   █▄█ █▀█ █▄▄ █░█ █▀▀ █▄█ ░█░   ▄ ▄
     )";
break;
     case 5:
        cout<<R"(
     
██╗░░░██╗░█████╗░██╗░░░██╗██╗██╗░░░██╗███████╗  ░██╗░░░░░░░██╗██╗███╗░░██╗  ████████╗██╗░░██╗███████╗
╚██╗░██╔╝██╔══██╗██║░░░██║╚█║██║░░░██║██╔════╝  ░██║░░██╗░░██║██║████╗░██║  ╚══██╔══╝██║░░██║██╔════╝
░╚████╔╝░██║░░██║██║░░░██║░╚╝╚██╗░██╔╝█████╗░░  ░╚██╗████╗██╔╝██║██╔██╗██║  ░░░██║░░░███████║█████╗░░
░░╚██╔╝░░██║░░██║██║░░░██║░░░░╚████╔╝░██╔══╝░░  ░░████╔═████║░██║██║╚████║  ░░░██║░░░██╔══██║██╔══╝░░
░░░██║░░░╚█████╔╝╚██████╔╝░░░░░╚██╔╝░░███████╗  ░░╚██╔╝░╚██╔╝░██║██║░╚███║  ░░░██║░░░██║░░██║███████╗
░░░╚═╝░░░░╚════╝░░╚═════╝░░░░░░░╚═╝░░░╚══════╝  ░░░╚═╝░░░╚═╝░░╚═╝╚═╝░░╚══╝  ░░░╚═╝░░░╚═╝░░╚═╝╚══════╝

                        ░██████╗░░█████╗░███╗░░░███╗███████╗  ██╗██╗
                        ██╔════╝░██╔══██╗████╗░████║██╔════╝  ██║██║
                        ██║░░██╗░███████║██╔████╔██║█████╗░░  ██║██║
                        ██║░░╚██╗██╔══██║██║╚██╔╝██║██╔══╝░░  ╚═╝╚═╝
                        ╚██████╔╝██║░░██║██║░╚═╝░██║███████╗  ██╗██╗
                        ░╚═════╝░╚═╝░░╚═╝╚═╝░░░░░╚═╝╚══════╝  ╚═╝╚═╝
        
     )";
     break;
    }
}

void levelTxt(int option){
switch(option){


    case 1:
        cout<<R"(

█▀▀ ▄▀█ █▀▄▀█ █▀▀   █▀█ █░█ █▀▀ █▀█
█▄█ █▀█ █░▀░█ ██▄   █▄█ ▀▄▀ ██▄ █▀▄

     )";
     break;

     case 2:
        cout<<R"(


█▄█ █▀█ █░█   █░█░█ █ █▄░█   █
░█░ █▄█ █▄█   ▀▄▀▄▀ █ █░▀█   ▄
     )";
     break;

     case 3:
        cout<<R"(


█▀▄ █▀█   █▄█ █▀█ █░█   █░█░█ ▄▀█ █▄░█ ▀█▀   ▀█▀ █▀█   ▀█▀ ▄▀█ █▄▀ █▀▀   ▄▀█   █▀ █░█ █▀█ █▀█ ▀█▀ █▀▀ █░█ ▀█▀   ▀█
█▄▀ █▄█   ░█░ █▄█ █▄█   ▀▄▀▄▀ █▀█ █░▀█ ░█░   ░█░ █▄█   ░█░ █▀█ █░█ ██▄   █▀█   ▄█ █▀█ █▄█ █▀▄ ░█░ █▄▄ █▄█ ░█░   ░▄
     )";
     break;

     case 4:
        cout<<R"(

█▀▄ █▀█   █▄█ █▀█ █░█   █░█░█ ▄▀█ █▄░█ ▀█▀   ▀█▀ █▀█   ▀█▀ ▄▀█ █▄▀ █▀▀  
█▄▀ █▄█   ░█░ █▄█ █▄█   ▀▄▀▄▀ █▀█ █░▀█ ░█░   ░█░ █▄█   ░█░ █▀█ █░█ ██▄  

▄▀█ █▄░█   ▄▀█ █░░ ▀█▀ █▀▀ █▀█   █▀█ ▄▀█ ▀█▀ █░█   ▀█
█▀█ █░▀█   █▀█ █▄▄ ░█░ ██▄ █▀▄   █▀▀ █▀█ ░█░ █▀█   ░▄

█▄█ █▀█ █░█   █▀▀ █▀█ █░█ █░░ █▀▄   █░█░█ █ █▄░█   █▄▄ █▀▀ ▀█▀ ▀█▀ █▀▀ █▀█   █▀█ █▀▀ ▄▀█ █░█░█ ▄▀█ █▀█ █▀▄ █▀   ▀▄ ▀ █▀▄
░█░ █▄█ █▄█   █▄▄ █▄█ █▄█ █▄▄ █▄▀   ▀▄▀▄▀ █ █░▀█   █▄█ ██▄ ░█░ ░█░ ██▄ █▀▄   █▀▄ ██▄ █▀█ ▀▄▀▄▀ █▀█ █▀▄ █▄▀ ▄█   ▄▀ ▄ █▄▀

     )";
     break;

     case 5:
        cout<<R"(


█▀▀ █▄░█ █▀▀ █▀▄▀█ █ █▀▀ █▀   █▀▀ █▀█ █▄░█ ▄▀█   █▄▄ █▀▀ █▀▄▀█ █▀█ █▀█ █▀▀   █▀▄ ▄▀█ █▄░█ █▀▀ █▀▀ █▀█ █▀█ █░█ █▀  
██▄ █░▀█ ██▄ █░▀░█ █ ██▄ ▄█   █▄█ █▄█ █░▀█ █▀█   █▄█ ██▄ █░▀░█ █▄█ █▀▄ ██▄   █▄▀ █▀█ █░▀█ █▄█ ██▄ █▀▄ █▄█ █▄█ ▄█  

                              ▄█ ░   █▄█ █▀▀ █▀           ▀█ ░   █▄░█ █▀█
                              ░█ ▄   ░█░ ██▄ ▄█           █▄ ▄   █░▀█ █▄█
     )";
     break;

     case 6:
        cout<<R"(


█▀▀ █▄░█ ▀█▀ █▀▀ █▀█ █ █▄░█ █▀▀   ▀█▀ █░█ █▀▀   █▀ █░█ █▀█ █▀█ ▀█▀ █▀▀ █░█ ▀█▀   ▀▄ ▀ ▀▄
██▄ █░▀█ ░█░ ██▄ █▀▄ █ █░▀█ █▄█   ░█░ █▀█ ██▄   ▄█ █▀█ █▄█ █▀▄ ░█░ █▄▄ █▄█ ░█░   ▄▀ ▄ ▄▀
     )";
     break;

     case 7:
        cout<<R"(


█▀▀ █▀█ █▄░█ ▀█▀ █ █▄░█ █░█ █ █▄░█ █▀▀   █▄░█ █▀█ █▀█ █▀▄▀█ ▄▀█ █░░   █▀█ ▄▀█ ▀█▀ █░█   ░ ░ ░
█▄▄ █▄█ █░▀█ ░█░ █ █░▀█ █▄█ █ █░▀█ █▄█   █░▀█ █▄█ █▀▄ █░▀░█ █▀█ █▄▄   █▀▀ █▀█ ░█░ █▀█   ▄ ▄ ▄
     )";
     break;


    case 8:
        cout<<R"(
     
██╗░░░██╗░█████╗░██╗░░░██╗██╗██╗░░░██╗███████╗  ░██╗░░░░░░░██╗██╗███╗░░██╗  ████████╗██╗░░██╗███████╗
╚██╗░██╔╝██╔══██╗██║░░░██║╚█║██║░░░██║██╔════╝  ░██║░░██╗░░██║██║████╗░██║  ╚══██╔══╝██║░░██║██╔════╝
░╚████╔╝░██║░░██║██║░░░██║░╚╝╚██╗░██╔╝█████╗░░  ░╚██╗████╗██╔╝██║██╔██╗██║  ░░░██║░░░███████║█████╗░░
░░╚██╔╝░░██║░░██║██║░░░██║░░░░╚████╔╝░██╔══╝░░  ░░████╔═████║░██║██║╚████║  ░░░██║░░░██╔══██║██╔══╝░░
░░░██║░░░╚█████╔╝╚██████╔╝░░░░░╚██╔╝░░███████╗  ░░╚██╔╝░╚██╔╝░██║██║░╚███║  ░░░██║░░░██║░░██║███████╗
░░░╚═╝░░░░╚════╝░░╚═════╝░░░░░░░╚═╝░░░╚══════╝  ░░░╚═╝░░░╚═╝░░╚═╝╚═╝░░╚══╝  ░░░╚═╝░░░╚═╝░░╚═╝╚══════╝

                        ░██████╗░░█████╗░███╗░░░███╗███████╗  ██╗██╗
                        ██╔════╝░██╔══██╗████╗░████║██╔════╝  ██║██║
                        ██║░░██╗░███████║██╔████╔██║█████╗░░  ██║██║
                        ██║░░╚██╗██╔══██║██║╚██╔╝██║██╔══╝░░  ╚═╝╚═╝
                        ╚██████╔╝██║░░██║██║░╚═╝░██║███████╗  ██╗██╗
                        ░╚═════╝░╚═╝░░╚═╝╚═╝░░░░░╚═╝╚══════╝  ╚═╝╚═╝
        
     )";
break;
     case 9:
        cout<<R"(

█▀▄ █▀█   █▄█ █▀█ █░█   █░█░█ ▄▀█ █▄░█ ▀█▀   ▀█▀ █▀█   █▀█ █▀▀ █▀ █▀▀ ▀█▀   █▄█ █▀█ █░█ █▀█
█▄▀ █▄█   ░█░ █▄█ █▄█   ▀▄▀▄▀ █▀█ █░▀█ ░█░   ░█░ █▄█   █▀▄ ██▄ ▄█ ██▄ ░█░   ░█░ █▄█ █▄█ █▀▄

█▀█ █▀█ █▀█ █▀▀ █▀█ █▀▀ █▀ █▀   ▀█
█▀▀ █▀▄ █▄█ █▄█ █▀▄ ██▄ ▄█ ▄█   ░▄



▄█ ░   █▄█ █▀▀ █▀         ▀█ ░   █▄░█ █▀█
░█ ▄   ░█░ ██▄ ▄█         █▄ ▄   █░▀█ █▄█
        
     )";
break;
     case 10:
        cout<<R"(

        
█░░ █▀▀ ▄▀█ █░█ █ █▄░█ █▀▀   ▀█▀ █░█ █▀▀   █▀▀ ▄▀█ █▀▄▀█ █▀▀   ░ ░ ░
█▄▄ ██▄ █▀█ ▀▄▀ █ █░▀█ █▄█   ░█░ █▀█ ██▄   █▄█ █▀█ █░▀░█ ██▄   ▄ ▄ ▄
     )";
break;
    case 11:
        cout<<R"(

        
█▀█ █▀▀ █▀ ▀█▀ ▄▀█ █▀█ ▀█▀ █ █▄░█ █▀▀   █▀█ █▀█ █▀█ █▀▀ █▀█ █▀▀ █▀ █▀   ░ ░ ░
█▀▄ ██▄ ▄█ ░█░ █▀█ █▀▄ ░█░ █ █░▀█ █▄█   █▀▀ █▀▄ █▄█ █▄█ █▀▄ ██▄ ▄█ ▄█   ▄ ▄ ▄
     )";
break;
     case 12:
        cout<<R"(

        
█▀▄ ▄▀█ ▀█▀ ▄▀█   █▀▀ █▀█ █▀█ █▀█ █▀█   █
█▄▀ █▀█ ░█░ █▀█   ██▄ █▀▄ █▀▄ █▄█ █▀▄   ▄
     )";
     break;
}
}

void playerTxt(){
    
    cout << R"(
    
    
█▄█ █▀█ █░█   █░█ ▄▀█ █░█ █▀▀ █▄░█ ▀ ▀█▀   █ ▀█▀ █▀▀ █▀▄▀█ █▀   ▀█▀ █▀█   █░█ █▀ █▀▀
░█░ █▄█ █▄█   █▀█ █▀█ ▀▄▀ ██▄ █░▀█ ░ ░█░   █ ░█░ ██▄ █░▀░█ ▄█   ░█░ █▄█   █▄█ ▄█ ██▄
    
    )";
}

void utilitesTxt(int option){

    switch(option){


 case 1:
        cout<<R"(

█▀ ▄▀█ █░█ █▀▀ █▀▄   █
▄█ █▀█ ▀▄▀ ██▄ █▄▀   ▄
     )";
break;
      case 2:
        cout<<R"(

█▀ ▄▀█ █░█ █▀▀   █▀▀ █▀█ █▀█ █▀█ █▀█
▄█ █▀█ ▀▄▀ ██▄   ██▄ █▀▄ █▀▄ █▄█ █▀▄
     )";
break;
      case 3:
        cout<<R"(

█░░ █▀█ ▄▀█ █▀▄   █▀▀ █▀█ █▀█ █▀█ █▀█   ▀   █ █▄░█ █░█ ▄▀█ █░░ █ █▀▄   █▀▀ █ █░░ █▀▀   █▀▀ █▀█ █▀█ █▀▄▀█ ▄▀█ ▀█▀
█▄▄ █▄█ █▀█ █▄▀   ██▄ █▀▄ █▀▄ █▄█ █▀▄   ▄   █ █░▀█ ▀▄▀ █▀█ █▄▄ █ █▄▀   █▀░ █ █▄▄ ██▄   █▀░ █▄█ █▀▄ █░▀░█ █▀█ ░█░
     )";
break;
case 4:
        cout<<R"(

█░░ █▀█ ▄▀█ █▀▄
█▄▄ █▄█ █▀█ █▄▀
     )";
break;
    }
}





